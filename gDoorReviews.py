...
def getPages(url, head):
    ...
    if nextPage is None:
        print(f"Could not find the element with class 'flex-grid tbl margTop' in {url}")
        return pages

    for link in nextPage.findAll('a'):
        if 'href' in link.attrs:
            new_url = 'https://glassdoor.com' + link.attrs['href']
            if new_url not in pages:
                pages.add(new_url)

    # Get last page
    for lastPage in nextPage.findAll('li', {'class': 'page last'}):
        lastPageUrl = 'https://glassdoor.com' + lastPage.a['href']
        getPages(lastPageUrl, head)
